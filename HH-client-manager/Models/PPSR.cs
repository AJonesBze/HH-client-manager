using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace HH_client_manager.Models
{
    public class PPSR
    { 
     public String ClientNumber { get; set; }
    public DateTime DateToday { get; set; }
    public String Ethnicity { get; set; }
    public int Age { get; set; }
    public String Assessment_Type { get; set; }
    public int Response1 { get; set; }
    public int Response2 { get; set; }
    public int Response3 { get; set; }
    public int Response4 { get; set; }
    public int Response5 { get; set; }
    public int Response6 { get; set; }
    public int Response7 { get; set; }
    public int Response8 { get; set; }
    public int Response9 { get; set; }
    public int Response10 { get; set; }
    public int Response11 { get; set; }
    public int Response12 { get; set; }
    public int Response13 { get; set; }
    public int Response14 { get; set; }
    public int Response15 { get; set; }
    public int Response16 { get; set; }
    public int Response17 { get; set; }
    public int Response18 { get; set; }
        public int Response19 { get; set; }
        public int Response20 { get; set; }
        public int Response21 { get; set; }
        public int Response22 { get; set; }
        public int Response23 { get; set; }
        public int Response24 { get; set; }
        public int Response25 { get; set; }
        public int Response26 { get; set; }
        public int Response27 { get; set; }
        public int Response28 { get; set; }
    }
}
